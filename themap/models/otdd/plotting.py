...
def gaussian_density_plot(
    ...
    ax.contourf(xx, yy, f, levels, cmap=cmap, alpha=0.5)
...
def heatmap(data, row_labels, col_labels, ax=None, cbar=True, cbar_kw={}, cbarlabel="", **kwargs):
...
def dist_adapt_regplot(
    ...
    if annotate:
        ...
        if annotation_arrows:
            adjust_text(
                ...
                arrowprops=dict(arrowstyle="-", color=arrowcolor, alpha=0.5, lw=0.5),
            )
        else:
    ...
    if show_correlation:
        ...
        )  # ,borderpad=0.2,handletextpad=0, handlelength=0) # using a size in points
        ax.legend(
            [ax.get_lines()[0]], ax.get_legend_handles_labels()[-1], handlelength=1.0, loc="best"
        )  # , handletextpad=0.0)
    ...
    **kwargs : dict
    ...
    for i in range(xs.shape[0]):
        for j in range(xt.shape[0]):
            if G[i, j] / mx > thr:
                ax.plot([xs[i, 0], xt[j, 0]], [xs[i, 1], xt[j, 1]], alpha=G[i, j] / mx, **kwargs)
...
def annotate_group(name, span, ax=None, orient="h", side=None):
    ...
    def annotate(ax, name, left, right, y, pad):
        ...
        arrow = ax.annotate(
            ...
            linespacing=2.0,
            arrowprops=dict(arrowstyle="-", shrinkA=0, shrinkB=0, connectionstyle=connectionstyle),
            fontsize=8,
            rotation=rot,
        )
        return arrow
...
def method_comparison_plot(
    df, hue_var="method", style_var="method", figsize=(15, 4), ax=None, save_path=None
):
    """Produce plots comparing OTDD variants in terms of runtime and distance"""
    if ax is None:
        fig, ax = plt.subplots(1, 2, figsize=figsize)

    lplot_args = {"hue": hue_var, "style": style_var, "data": df, "x": "n", "markers": True}
