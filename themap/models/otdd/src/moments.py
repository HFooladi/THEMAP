...
from .utils import extract_data_targets, process_device_arg
from .wasserstein import bures_distance
...
class OnlineStatsRecorder:
    ...
    def update(self, batch):
        ...
        if self.n == 0:
            ...
            if self.diagonal_cov and self.centered_cov:
                self.Σ = (
                    (m - 1) * self.Σ + ((m - 1) / (m + n - 1)) * ((batch - self.μ).pow(2).sum(axis=0))
                ) / (m + n - 1)
            elif self.diagonal_cov and not self.centered_cov:
                self.Σ = (m - 1) / (m + n - 1) * self.Σ + 1 / (m + n - 1) * (batch.pow(2).sum(axis=0))
            elif self.centered_cov:
                self.Σ = (
                    (m - 1) * self.Σ + ((m - 1) / (m + n - 1)) * ((batch - self.μ).T).matmul(batch - self.μ)
                ) / (m + n - 1)
            else:
    ...
    def retrieve(self, verbose=False):
        ...
        if verbose:
            print("Mean and Covariance computed on {} samples".format(int(self.n)))
...
def _single_label_stats(
    ...
    if len(label_indices) < 2:
        logger.warning(
            " -- Class '{:10}' has too few examples ({}). Ignoring it.".format(c, len(label_indices))
        )
        if M is None:
        ...
        if isinstance(data, dataloader.DataLoader):
            ...
            filtered_loader = dataloader.DataLoader(
                data, batch_size=batch_size, sampler=SubsetRandomSampler(label_indices)
            )
            _orig_indices = None

        if online:
            ## Will compute online (i.e. without loading all the data at once)
            stats_rec = OnlineStatsRecorder(
                centered_cov=True,
                twopass=True,
        ...
        if _orig_indices is not None:
            data.sampler.indices = _orig_indices
...
def dimreduce_means_covs(Means, Covs, redtype="diagonal"):
    ...
    if redtype == "diagonal":
        ...
        Covs[0] = MDS(n_components=k).fit_transform(Covs[0].view(Covs[0].shape[0], -1))
        Covs[1] = MDS(n_components=k).fit_transform(Covs[1].view(Covs[1].shape[0], -1))
        ...
        Covs = [U, V]
        print("Done! Σ's Dimensions: {} (Task 1) and {} (Task 2)".format(list(U.shape), list(V.shape)))
    else:
...
def pairwise_distance_mse(U, V, D, reg=1):
    ...
    loss = torch.norm(D - d_uv) ** 2 / D.numel() + reg * (
        torch.norm(U) ** 2 / U.numel() + torch.norm(V) ** 2 / V.numel()
    )  # MSE per entry
    return loss
...
def bipartite_mse_embedding(D, k=100, niters=10000):
    ...
    print("Final distortion: ||D - D'||\u00b2/|D| = {:4.2f}".format(loss.item()))
