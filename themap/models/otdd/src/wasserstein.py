...
def bures_distance(Σ1, Σ2, sqrtΣ1, commute=False, squared=True):
    ...
    if not commute:
        ...
        bures = torch.trace(Σ1 + Σ2 - 2 * sqrtm(torch.mm(torch.mm(sqrtΣ1, Σ2), sqrtΣ1)))
    else:
...
def bbures_distance(
    ...
    if sqrtΣ1 is None and not diagonal_cov:
        sqrtΣ1 = (
            sqrtm(Σ1) if sqrt_method == "spectral" else sqrtm_newton_schulz(Σ1, sqrt_niters)
        )  # , return_inverse=True)
    ...
    if diagonal_cov:
        ...
        if sqrt_method == "spectral":
            ...
            cross = sqrtm_newton_schulz(torch.matmul(torch.matmul(sqrtΣ1, Σ2), sqrtΣ1), sqrt_niters)
...
def wasserstein_gauss_distance(
    μ_1, μ_2, Σ1, Σ2, sqrtΣ1=None, cost_function="euclidean", squared=False, **kwargs
):
    """
...
def pwdist_gauss(M1, S1, M2, S2, symmetric=False, return_dmeans=False, nworkers=1, commute=False):
    ...
    D = torch.zeros((n1, n2)).to(M1[0].device if len(M1) > 0 else "cpu")
    ...
    if not sqrtS:
        ...
        for i, Σ in tqdm(enumerate(S), leave=False):
            if diagonal_cov:
                ...
                sqrtS.append(sqrtm(Σ) if sqrt_method == "spectral" else sqrtm_newton_schulz(Σ, sqrt_niters))
    ...
    if not means_only:
        for i, j in pbar:
            if means_only:
                ...
                D[i, j] = ((M1[i] - M2[j]) ** 2).sum(axis=-1) + (S1[i] + S2[j]).diagonal(
                    dim1=-2, dim2=-1
                ).sum(-1)
            if symmetric:
                D[j, i] = D[i, j]
