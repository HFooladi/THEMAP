...
def sqrtm_newton_schulz(A, numIters, reg=None, return_error=False, return_inverse=False):
    ...
    eye_matrix = torch.eye(dim, dim).view(1, dim, dim).repeat(batchSize, 1, 1).to(A.device)  # .type(dtype)
    Z = torch.eye(dim, dim).view(1, dim, dim).repeat(batchSize, 1, 1).to(A.device)  # .type(dtype)
    for i in range(numIters):
        T = 0.5 * (3.0 * eye_matrix - Z.bmm(Y))
        Y = Y.bmm(T)
        Z = T.bmm(Z)
...
def create_symm_matrix(batchSize, dim, numPts=20, tau=1.0, dtype=torch.float32, verbose=False):
