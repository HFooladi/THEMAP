...
from .utils import (
    augmented_dataset,
    extract_data_targets,
    load_full_dataset,
    multiclass_hinge_loss,
)
from .wasserstein import efficient_pwdist_gauss, pwdist_exact, pwdist_means_only, pwdist_upperbound
...
class DatasetDistance:
    ...
    def _load_infer_labels(self, D, classes=None, reindex=None, reindex_start=None):
        if classes:
            ...
            def labeling_fun(X):
                return torch.LongTensor(k_means(X.numpy(), k)[1])
        else:

            def labeling_fun(X):
                return torch.LongTensor(DBSCAN(eps=5, min_samples=4).fit(X).labels_)
    ...
    def _init_data(self, D1, D2):
        ...
        if (targets1 is None) or self.ignore_source_labels:  # Unsupervised setting
            ...
            if Y_true is not None:
                self.Y1_true = Y_true  # will not be used by OTDD, stored only for downstream eval
        else:
    ...
    def _load_datasets(self, maxsamples=None, device=None):
        ...
        logger.info(" * D1 = {} x {} ({} unique labels)".format(*self.X1.shape, len(self.V1)))
        logger.info(" * D2 = {} x {} ({} unique labels)".format(*self.X2.shape, len(self.V2)))
    ...
    def _get_label_stats(self, side="both"):
        ...
        if (None not in self.Means) and (None not in self.Covs):
        ...
        if (side == "both" or side == "src") and (self.Means[0] is None):
            ...
            M1, C1 = compute_label_stats(
                self.D1, self.targets1, self.idxs1, self.classes1, embedding=self.src_embedding, **shared_args
            )
    ...
    def _get_label_distances(self):
        ...
        if self.inner_ot_method == "gaussian_approx":
            ...
            pwdist = partial(pwdist_means_only, symmetric=self.symmetric_tasks, device=self.device)
        ...
        if self.debiased_loss and self.symmetric_tasks:
            ...
            if self.inner_ot_method in ["gaussian_approx", "naive_upperbound", "means_only"]:
                DYY1_means, DYY2_means = DYY12_means, DYY12_means

        if self.debiased_loss:
            D = torch.cat([torch.cat([DYY1, DYY12], 1), torch.cat([DYY12.t(), DYY2], 1)], 0)
            if self.inner_ot_method in ["gaussian_approx", "naive_upperbound", "means_only"]:
                D_means = torch.cat(
                    [torch.cat([DYY1_means, DYY12_means], 1), torch.cat([DYY12_means.t(), DYY2_means], 1)], 0
                )
        else:
            D = DYY12
            if self.inner_ot_method in ["gaussian_approx", "naive_upperbound", "means_only"]:
        ...
        if self.method == "augmentation":
            ...
            if self.λ_x != 1.0 or self.λ_y != 1.0:
                raise NotImplementedError(
                    "Unevenly weighted feature/labelnot available for method=augmentation yet"
                )
            ...
            if maxsamples and self.X1.shape[0] > maxsamples:
                idxs_1 = sorted(np.random.choice(self.X1.shape[0], maxsamples, replace=False))
            else:
                idxs_1 = np.s_[:]  # hack to get a full slice

            if maxsamples and self.X2.shape[0] > maxsamples:
                idxs_2 = sorted(np.random.choice(self.X2.shape[0], maxsamples, replace=False))
            else:
                idxs_2 = np.s_[:]  # hack to get a full slice

            Z1 = torch.cat((self.X1[idxs_1], self.Y1[idxs_1].type(self.X1.dtype).unsqueeze(1)), -1)
            Z2 = torch.cat((self.X2[idxs_2], self.Y2[idxs_2].type(self.X2.dtype).unsqueeze(1)), -1)
            ...
            cost_geomloss = partial(
                batch_augmented_cost, W=W, λ_x=self.λ_x, λ_y=self.λ_y, feature_cost=self.feature_cost
            )
    ...
    def subgroup_distance(self, labels_a, labels_b, maxsamples=500):
        ...
        if self.method == "augmentation":
            ...
            del DB
            pdb.set_trace()
            loss = geomloss.SamplesLoss(loss="sinkhorn", p=2, blur=self.entreg ** (1 / self.p))
            ## By default, use constant weights = 1/number of samples
            dist = loss(XA, XB)
            del XA, XB
        elif self.method == "precomputed_labeldist":
            ## This one leverages precomputed pairwise label distances
            cost_geomloss = partial(
                batch_augmented_cost, W=self._get_label_distances(), V1=len(self.V1), V2=len(self.V2)
            )
            ...
            if maxsamples and mask_a.sum() > maxsamples:
                idxs_a = idxs_a[sorted(np.random.choice(len(idxs_a), maxsamples, replace=False))]
            if maxsamples and mask_b.sum() > maxsamples:
                idxs_b = idxs_b[sorted(np.random.choice(len(idxs_b), maxsamples, replace=False))]
    ...
    def plot_label_stats(
        ...
        for i in range(2):
            if i == 1 and not show_target:
                continue
            X, Y, c = (self.X1, self.Y1, self.classes1) if i == 0 else (self.X2, self.Y2, self.classes2)
            X = X.clone().detach()  # In case we're in dynamic setting
            Y = Y.clone().detach()
            axi = ax[i] if ncol == 2 else ax
            if same_plot:
                axi.set_xlim(min(lims["x"][0][0], lims["x"][1][0]), max(lims["x"][0][1], lims["x"][1][1]))
                axi.set_ylim(min(lims["y"][0][0], lims["y"][1][0]), max(lims["y"][0][1], lims["y"][1][1]))
            else:
            ...
            for j, (μ, Σ) in enumerate(zip(Means[i], Covs[i])):
                ...
                axi.scatter(
                    X[:, 0], X[:, 1], marker=markers[i], s=8, c=scatter_colors[i], cmap=scatter_cmaps[i]
                )  # , normalize = )
    ...
    def plot_coupling(
        ...
        if boundaries:
            ...
            imshow_group_boundaries(ax, src_group_sizes, trg_group_sizes, group_names=gnames)
...
class IncomparableDatasetDistance(DatasetDistance):
    ...
    def _get_label_distances(self):
        ...
        if self._pwlabel_stats_1 is not None:
        ...
        if self._pwlabel_stats_2 is not None:
            logger.info("Found pre-existing D2 label-label stats, will not recompute")
    ...
    def distance(self, maxsamples=10000, return_coupling=False):
        ...
        π, log = ot.gromov.entropic_gromov_wasserstein(
            C1, C2, a, b, loss_fun="square_loss", epsilon=self.entreg, log=True, verbose=True
        )
        dist = log["gw_dist"]
...
class FeatureCost:
    ...
    def __init__(self, src_embedding=None, tgt_embedding=None, src_dim=None, tgt_dim=None, p=2, device="cpu"):
    ...
    def __call__(self, X1, X2):
        ...
        if self.src_emb is not None:
            ...
            try:
                ...
                X1 = self._batchify_computation(X1.view(-1, *self.src_dim).to(self.device), "x").reshape(
                    B1, N1, -1
                )
        if self.tgt_emb is not None:
            ...
            try:
                ...
                print("Batchifying feature distance computation")
                X2 = self._batchify_computation(X2.view(-1, *self.tgt_dim).to(self.device), "y").reshape(
                    B2, N2, -1
                )
        if self.p == 1:
...
def batch_augmented_cost(Z1, Z2, W=None, Means=None, Covs=None, feature_cost=None, p=2, λ_x=1.0, λ_y=1.0):
