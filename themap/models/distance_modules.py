...
class ProteinIdentityDistance:
    def __init__(self, seq1: str, seq2: str) -> None:
        logger.debug(
            f"Initializing ProteinIdentityDistance with sequences of lengths {len(seq1)} and {len(seq2)}"
        )
        self.seq1 = seq1
        self.seq2 = seq2
    ...
    def get_distance(self) -> float:
        ...
        self.seqid = float(self.exact_match) / min(self.query_length, self.target_length)
...
def get_seqid(aligner: Align.PairwiseAligner, target: str, query: str) -> float:
    ...
    seqid = float(exact_match) / min(query_length, target_length)
...
def max_seqid(assay_sequence: Dict[str, Any], pdbbinds_list: List[Dict[str, Any]]) -> Dict[str, Any]:
    logger.info(
        f"Finding maximum sequence identity for assay {assay_sequence.get('uniprot_id', 'unknown')} against {len(pdbbinds_list)} PDB structures"
    )
        ...
        try:
            ...
            for letter in assay_sequence["sequence"]:
                if letter not in aligner.alphabet:
                    ...
                    logger.info(
                        f"Replaced unknown amino acid {letter} with X in sequence {assay_sequence.get('uniprot_id', 'unknown')}"
                    )

            try:
                new_seqid = get_seqid(aligner, pdbbind["seq"], modified_sequence)
                if new_seqid > seqid:
                    seqid = float(new_seqid)
                    nearest_pdbbind = pdbbind["id"]
                    logger.debug(
                        f"Found new best match after sequence modification: {pdbbind['id']} with sequence identity {seqid:.4f}"
                    )
            except Exception as e:
                logger.error(
                    f"Failed to process PDB structure {pdbbind.get('id', 'unknown')} even after sequence modification: {str(e)}"
                )

    assay_sequence["seqid"] = seqid
    assay_sequence["nearest_pdbbind"] = nearest_pdbbind
    logger.info(
        f"Completed processing assay {assay_sequence.get('uniprot_id', 'unknown')}. Best match: {nearest_pdbbind} with sequence identity {seqid:.4f}"
    )
