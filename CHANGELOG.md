...
### Fixed
- **Featurizer tests skipped DGL on the wrong signal.** `test_all_featurizers_in_available_list` and `test_get_featurizer_routes_all` gated the `gin_supervised_*` featurizers on whether the `dgl` library imports. Since molfeat 1.0.0 removed `PretrainedDGLTransformer`, `dgl` being importable no longer means a DGL featurizer can be built — so Python 3.11 (real `dgl==1.1.3`) ran them and failed, while 3.12 passed only because its resolved `dgl==0.1.3` is too old to import. The guard now requires molfeat's transformer class as well.
- **`molfeat` is bounded to `<2`.** The requirement was an open-ended `>=0.10.0`, which is how molfeat 1.0.0 — a release that removed public API THEMAP depends on — was picked up silently five weeks after the last green CI run. The bound is a no-op today (Python 3.10 still resolves 0.11.0, 3.11+ still resolve 1.0.0) and makes the next major a deliberate upgrade rather than a surprise.
- **`mordredcommunity` is now a direct dependency.** THEMAP advertises `mordred` in `DESCRIPTOR_FEATURIZERS` but relied on molfeat installing `mordredcommunity` transitively. molfeat 1.0.0 moved it to an optional extra, so the `mordred` featurizer disappeared on Python 3.11+. It is declared explicitly in the `ml` and `all` extras instead of depending on molfeat's extra names staying stable.
